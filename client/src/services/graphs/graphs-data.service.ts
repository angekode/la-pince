...
export async function getCategoryTotalsForPieGraphData(): Promise<CategoryTotalsData[]> {
  const transactions = await getAllTransactions();
 ...
 */
export async function getSoldeEvolutionForCurveGraphData(): Promise<SoldeEvolutionData> {
  const transactions = await getAllTransactions();
